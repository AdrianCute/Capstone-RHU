...
if(isset($_POST['exportdata']))
{
    ...
    WHERE Status ='Complete' AND Date BETWEEN ' $from' AND '$to';
    ";
    ...
    if(mysqli_num_rows($query_run) > 0)
    {
        ...
        $sheet->setCellValue('H1', 'Medicine given');
        $sheet->setCellValue('I1', 'tooth extracted');
        ...
        foreach($query_run as $data)
        {
            ...
            $sheet->setCellValue('H'.$rowCount, $data['Medicine_given']);
            $sheet->setCellValue('I'.$rowCount, $data['tooth']);
        ...
        $sheet->getStyle('A1:I1')->applyFromArray($style);      
          $sheet->getStyle('A1:I1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_GRADIENT_LINEAR);
        $sheet->getStyle('A1:I1')->getFill()->setRotation(0);
        $sheet->getStyle('A1:I1')->getFill()->setStartColor($goldAccent4Lighter60['startcolor']);
        $sheet->getStyle('A1:I1')->getFill()->setEndColor($goldAccent4Lighter60['endcolor']);
