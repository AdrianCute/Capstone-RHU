					...
					document.querySelector('.full-container').style.display="none";
						...
						<table  id="filteredData" class="display emp-table" style="width: 100%;">
							<thead>
								...
								<th col-index = 16>Deferral </th>
								<th col-index = 17>Reason for Deferral</th>
								...
								<th col-index = 28>Adverse Event                          
									
								</th>      
								<th col-index = 29> Adverse Event Condition                                      
								</th> 
								<th col-index = 30> Row Hash                                    
									<select class="table-filter" onchange="filter_rows()">
										<option value="all"></option>
									</select>
								</th>
							    ...
							    if (mysqli_num_rows($result) > 0) { // Check if there are rows returned
								        while ($row = mysqli_fetch_assoc($result)) {
											...
											<input type="hidden" class="record-id" value="<?php echo $row["Record_ID"]; ?>">
								                ...
								                <td><?php echo $row["Deferral"] ?></td>
								                <td><?php echo $row["Reason_for_Deferral"] ?></td>
								                ...
								                <td><?php echo $row["Adverse_Event"] ?></td>
								                <td><?php echo $row["Adverse_Condition"] ?></td>
												<td><?php echo $row["Row_hash"] ?></td>
